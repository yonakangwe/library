...
func (con *StaffConn) Create(e *entity.Staff) (int32, error) {
	var StaffID int32
	query := `INSERT INTO ` + tableName + ` (fullName,email,phone,username,password_hash, created_by, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`
	...
	if util.IsError(err) {
		...
		log.Errorf("error creating staff from table %v: %v", tableName, err)
	}
	return StaffID, err
}
