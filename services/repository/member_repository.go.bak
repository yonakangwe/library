...
func (con *MemberConn) Create(e *entity.Member) (int32, error) {
	var memberID int32
	query := `INSERT INTO ` + memberTableName + ` (full_name, email, created_by, created_at) VALUES ($1, $2, $3, $4) RETURNING id`
	err := con.conn.QueryRow(context.Background(), query, e.FullName, e.Email, e.CreatedBy, time.Now()).Scan(&memberID)
	if util.IsError(err) {
...
// 			return nil, nil
// 		}
// 		log.Errorf("error getting  table %v: %v", memberTableName, err)
// 		return nil, err
// 	}
// 	Role := &entity.Role{
// 		ID:          id,
...
// 	query := `UPDATE ` + memberTableName + ` SET
// 										 name = $1,
// 										 description = $2,
// 										 updated_by = $3,
// 										 updated_at = $4
...
// 	if util.IsError(err) {
// 		log.Errorf("error updating  from table %v by id: %v", memberTableName, err)
// 	}
// 	return err
// }
...
// 	if util.IsError(err) {
// 		log.Errorf("error soft delete  from table %v by id: %v", memberTableName, err)
// 	}
// 	return err
// }

// func (con *MemberConn) HardDelete(id int32) error {
// 	query := `DELETE FROM ` + memberTableName + ` WHERE id = $1`
// 	_, err := con.conn.Exec(context.Background(), query, id)
// 	if util.IsError(err) {
// 		log.Errorf("error hard delete from table %v by id: %v", memberTableName, err)
// 	}
// 	return err
// }
...
// 	query := ` SELECT count(*) FROM ` + memberTableName + ` WHERE deleted_at IS NULL ` + innerQuery
// 	err := con.conn.QueryRow(context.Background(), query, args...).Scan(&totalCount)
// 	if err != nil {
// 		if err.Error() == error_message.ErrNoResultSet.Error() {
// 			return 0
// 		}
// 		log.Errorf("error getting total count from %v : %v", memberTableName, err)
// 	}
// 	return totalCount
// }
